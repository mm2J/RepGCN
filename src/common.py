...
def parse_option():
    ...
    parser.add_argument("--dataset", default="casia-b-query", choices=["casia-b-query", "casia-b"])
    parser.add_argument("--train_data_path", default="./data/casiab_npy", help="Path to train data CSV")
    parser.add_argument("--valid_data_path", default="./data/casia-b_npy", help="Path to validation data CSV")
    parser.add_argument("--train_id", default="1-74",
                        help="train_id")
    parser.add_argument("--test_id", default="75-124",
                        help="test_id")
    ...
    parser.add_argument("--arch", type=str, default='s72')
    parser.add_argument("--spatial_act", type=str, default='ReLU')
    parser.add_argument("--temporal_act", type=str, default='ReLU')
    parser.add_argument("--attn", type=str, default='oa')
    parser.add_argument("--rm_conf", type=str2bool,
                        nargs='?', default=True)
    parser.add_argument("--joint_drop", choices=["single", "none"], default="none") # rm drop arms
    parser.add_argument("--sampler_num_sample", type=int, default=8)
    parser.add_argument("--weight_path", help="Path to weights for model")
    parser.add_argument("--model_type", choices=["spatialtransformer_temporalconv", "spatiotemporal_transformer", "gaitgraph", "RepGCN"], default="RepGCN")
    # Optionals
    parser.add_argument("--num_workers", type=int, default=0)
    parser.add_argument("--theta", type=float, default=0.95, help="loss functions weight")
    parser.add_argument(
    ...
    parser.add_argument("--project", default="CASIA-B")
    parser.add_argument("--name", help="experiment_name", default="gait recognition for Ablation_Study_GSTC_Qg_Kg_Vg")
    ...
    parser.add_argument("--point_noise_std", type=float, default=0.1)
...
def get_trainer(model, opt, steps_per_epoch):
    optimizer = torch.optim.AdamW(model.parameters(), lr=opt.learning_rate, weight_decay=opt.weight_decay, betas=(0.9, 0.999), eps=1e-8)
    _optimizer_base = optimizer
    scaler = torch.cuda.amp.GradScaler(enabled=opt.use_amp)
    scheduler = torch.optim.lr_scheduler.OneCycleLR(_optimizer_base, opt.learning_rate, epochs=opt.epochs, steps_per_epoch=steps_per_epoch)

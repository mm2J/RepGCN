...
from models.RepGCN import RepGCN
...
def main(opt):
    ...
    elif opt.model_type == "RepGCN":
        model = RepGCN(in_channels=2 if opt.rm_conf else 3, spa_act_cfg=dict(type=opt.spatial_act), temp_act_cfg=dict(type=opt.temporal_act), arch=opt.arch, attn_cfg=opt.attn)
    else:
    ...
    conditions = ['NM#5-6', 'BG#1-2', 'CL#1-2']
    for epoch in tqdm(range(opt.start_epoch, opt.epochs + 1)):
        ...
        if epoch % opt.test_epoch_interval == 0 or epoch == opt.epochs:
            # evaluation
            accuracy_avg, sub_accuracies, dataframe = evaluate(
            ...
            view_accuracy = dataframe.to_dict()
            for view in views:
                for con in conditions:
                    unify_log.log({'accuracy/validation/'+con+'/'+str(view): view_accuracy[view][con]}, step=epoch)
